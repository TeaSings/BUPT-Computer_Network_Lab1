 ...
 *
 * DATA 帧仍带 CRC。
 * ACK/NAK 使用 3 字节短控制帧以降低确认开销。
 * 发送和接收缓存按 WINDOW_SIZE 做环形复用。
 * 定时器编号也映射到缓存槽位。
 * 误码恢复时优先针对坏帧头部携带的序号发送 NAK。
 * 该策略减少等待超时重传的时间。
 */
#define MAX_SEQ 63
...
/*
 * 缓存槽位按 seq % NR_BUFS 复用，
 * arrived 记录接收槽是否已有乱序帧。
 */
 ...
 * @param frame_expected DATA/ACK 调用时表示本端接收窗口左端；
 * NAK 调用时表示请求重传的序号。
 */
static void send_frame_datalink(unsigned char kind, unsigned char frame_nr,
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        case NETWORK_LAYER_READY: {
            ...
            nbuffered++;
            send_frame_datalink(FRAME_DATA, next_frame_to_send,
                                frame_expected);
        ...
        case FRAME_RECEIVED:
             ...
             */
            if (f.kind == FRAME_NAK &&
                between(ack_expected, f.ack, next_frame_to_send)) {
